...
export default async function ProjectDetail({
  ...
  if (!res.ok) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center px-8">
        <p className="text-2xl font-bold uppercase text-white mb-4">
          Project not found
        </p>
        <Link
          href="/#projects"
          className="text-raspberry uppercase text-sm tracking-widest hover:underline"
        >
          ← Back to projects
        </Link>
      </div>
  ...
  return (
    ...
    >
      <div
        className={`${isModal ? "min-h-screen bg-black text-white" : "min-h-screen bg-white text-black"}`}
      >
        <div className="pt-24 pb-16 px-8 md:px-16 max-w-screen-lg mx-auto">
          {/* Title */}
          <h1
            className={`text-4xl md:text-6xl font-bold uppercase mb-6 ${isModal ? "text-white" : "text-black"}`}
          >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 mt-10">
            <div className="flex flex-col gap-2">
              <p
                className={`text-sm ${isModal ? "text-gray-400" : "text-gray-600"}`}
              >
